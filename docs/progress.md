...
| 2026-03-04 | Claude Desktop導入・開発ツールのワークフロー整理 |
| 2026-03-13 | 外部作成のMeltyApp（melty-app.jsx）をフロントエンドに統合 |
| 2026-03-13 | スマホフレームを除去してPWA全画面表示に対応 |
| 2026-03-13 | Supabase Auth をフロントエンドから直接接続する方式（B案）を採用 |
| 2026-03-13 | ログイン・新規登録画面にSupabase認証処理を実装 |
| 2026-03-13 | 認証ガード（ProtectedRoute）を実装・未ログイン時は/loginへリダイレクト |
...
- [x] 共通レイアウト（下部ナビゲーションバー）← MeltyApp統合済み
- [x] 顧客一覧・詳細・編集・来店登録画面 ← MeltyApp統合済み（モックデータ）
- [x] カレンダー画面 ← MeltyApp統合済み（モックデータ）
- [x] 収入・成績画面 ← MeltyApp統合済み（モックデータ）
- [x] 設定画面 ← MeltyApp統合済み（モックデータ）
- [ ] Supabaseプロジェクト作成・`.env.local` に接続情報を設定
- [ ] モックデータをSupabase/FastAPI実データに差し替え
